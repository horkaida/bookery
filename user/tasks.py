...
def reading_time_statistic():
    ...
    for profile in profiles:
        ...
        total_reading_7days = ReadingSession.objects.filter(
            user=profile.user, stop_reading__gte=seven_days_ago).aggregate(duration=Sum(F('stop_reading') - F('start_reading'))
        )['duration']
        # Convert the total reading time to seconds, default to 0 if None
        total_reading_7days_seconds = total_reading_7days.total_seconds() if total_reading_7days else 0

        # Calculate total reading time for the last 30 days
        total_reading_30days = ReadingSession.objects.filter(
            user=profile.user, stop_reading__gte=thirty_days_ago).aggregate(duration=Sum(F('stop_reading')-F('start_reading'))
        )['duration']
        # Convert the total reading time to seconds, default to 0 if None
        total_reading_30days_seconds = total_reading_30days.total_seconds() if total_reading_30days else 0
