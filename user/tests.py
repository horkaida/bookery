...
class UserActivationTest(APITestCase):
    ...
    def test_account_activation(self):
        ...
        response = self.client.put(url, data={'uidb64': self.uidb64, 'token': self.token})
    ...
    def test_account_activation_invalid_token(self):
        ...
        """
        url = reverse('user-activate')
        response = self.client.put(url, data={'uidb64': self.uidb64, 'token': 'invalidtoken'})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class ChangeAndResetPasswordTest(APITestCase):
    ...
    def test_change_password(self):
        ...
        response = self.client.put(
            url, data={'current_password': 'strongpassword123',
    ...
    def test_change_password_current_password_is_incorrect(self):
        ...
        url = reverse('user-change-password')
        response = self.client.put(
            url, data={'current_password': 'incorectpass',
                       'new_password': 'newpasswordtest'},
            **self.auth_headers
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_change_password_same_as_old(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('New password cannot be the same as the old password.', response.data['detail'])
    ...
    def test_reset_password(self):
        ...
        response = self.client.put(
            url, data={'uidb64': uidb64, 'token': token, 'new_password': 'strongnewpassword'}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.user.refresh_from_db()
...
class ReadingStatisticTest(TestCase):
    ...
    def setUp(self):
        ...
        self.book = Book.objects.create(
            title='title', author='author', text='text', published='2000', short_description='short', full_description='full'

        )
        # Create reading sessions within and outside the 7 and 30 day windows
        now = timezone.now()
        self.reading_session_1 = ReadingSession.objects.create(
            user=self.user, book=self.book, start_reading=now - timedelta(days=5, hours=1), stop_reading=now - timedelta(days=5)
        )
        self.reading_session_2 = ReadingSession.objects.create(
            user=self.user, book=self.book, start_reading=now - timedelta(days=15, hours=2), stop_reading=now - timedelta(days=15)
        )
        self.reading_session_3 = ReadingSession.objects.create(
            user=self.user, book=self.book, start_reading=now - timedelta(days=35, hours=3), stop_reading=now - timedelta(days=35)
        )
    ...
    def test_reading_time_statistic_authorized(self):
        ...
        self.assertEqual(self.user.profile.total_reading_7days, 3600)  # 1 hour = 3600 seconds
        self.assertEqual(self.user.profile.total_reading_30days, 10800)  # 3 hours in total
